...
const cvFormatter = (arr) => {
    ...
    let newArr = arr.filter(arr => arr.yearsOfExperience > 1);
    let fullName;
...
const applicationsStatics = (arr) => {
    let result = {
      ...
      if(((element.firstName == null ) || (element.lastName == null)) || (element.yearsOfExperience == 0) ){
    ...
    function Average(arr){
     ...
     for (let i = 0; i < data.grades.length; i++) {
         for (let j = 0; j < data.grades[i].length; j++) {
                    ...
                    data.grades[i].classes[j].avg = Average(data.grades[i].classes[j].classScores);
