...
function capitalA(s){
    ...
    let containLower = s.match(/[a-z]/g);
    let containUpper = s.match(/[A-Z]/g);
    let containSymbot = s.match(/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/g);


    if (containLower != null && (containUpper !=null) && (endCapA == true) && (isNumber == null) && (containSymbot == null)) {
...
function ioEmail(email){
    let valEmail = email.match(/^[\w]+@([\w-]+\.)+[\w-]{2,4}$/g);
    if (valEmail != null ) {
        return true;
    }
    else{
